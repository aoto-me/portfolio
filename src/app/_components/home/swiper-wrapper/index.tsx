...
export const SwiperWrapper = ({ worksList }: SwiperWrapperProps) => {
  ...
  useEffect(() => {
    ...
    let stableCount = 0;
    let retries = 0;
    const maxRetries = 15; // 最大試行回数（4〜5秒くらい）
  ...
  const anchorSizeTimerRef = useRef<null | ReturnType<typeof setTimeout>>(null);

  // 遅延で計測してレイアウト確定前の誤計測を防ぐ
  const updateAnchorSize = useCallback(() => {
    if (anchorSizeTimerRef.current) clearTimeout(anchorSizeTimerRef.current);
    anchorSizeTimerRef.current = setTimeout(() => {
  ...
  const handleRotate = useCallback(() => {
    ...
    circleRef.current?.classList.add('isActive');

    const startDeg = degRef.current;
    const targetDeg = startDeg - 360; // 移動量
    const duration = 1900; // スピード調整(ミリ秒)
    ...
    const animate = (now: number) => {
      ...
      if (progress < 1) {
        ...
        circleRef.current?.classList.remove('isActive');
   ...
   * inert属性の更新
   */
  const updateSlidesInert = useCallback((swiper: SwiperType) => {
    ...
    >
      ...
      <Swiper
        ...
        onTransitionEnd={swiper => {
          // ドラッグキャンセルやトランジション割り込み時もinertを正しく更新するフォールバック
          updateSlidesInert(swiper);
        }}
          ...
          <SwiperSlide key={`works-${item.id}`}>
            <article>
              ...
              >
                ...
                <div className={clsx('imageMask', ready && 'isShow')} data-slide={index}>
                  <Image
      ...
      {/* 円形運動 */}
      <div className={clsx('circular', ready && 'isShow')}>
